class AgeLoan < ActiveRecord::Base
	...
	def self.fuzzy_attr
		@@fuzzy_attr ||= {
			age: [
				:young, [25, 1, 40, 0],
				:middle_aged, [25, 0, 40, 1, 55, 1, 60, 0],
				:old, [55, 0, 60, 1]
			],
			ratio: self.gen_fs(:ratio, 20, 90, 10),
			difference: [
	...
	def self.track_difference data, filter, par1, par2
		...
		{dir: sent[:diff][0], speed: [AgeLoan.find_best_fit_fs(:difference, sent[:diff][1])[0], sent[:diff][1]],
	...
	def self.summary? data, filter, summary
  	...
  	def self.find_best_fit data, filter, sum_param
  		...
  		fs, truth = shrink_fs(data, filter, sum_param, new_fs, summary?(data, filter, [[sum_param, new_fs]]), li, ri, 0)
  	...
  	def self.shrink_fs data, filter, target_param, cur_fs, truth, li, ri, level
  		...
  		return [cur_fs, truth, level] if reduced_l.empty? || reduced_r.empty?
	...
	def self.fs_to_s fs
		return fs.to_s.sub('_', ' ') if fs.is_a? Symbol
